...
require (
	...
	// LOCKED -- DO NOT BUMP naabu ABOVE v2.3.3.
	//
	// This pin is deliberate and predates the 2026-08 dependency sweep. Leave
	// it at v2.3.3 unless the person who owns the port-scanning behaviour has
	// explicitly signed off on moving it. If a vulnerability scanner flags
	// naabu, raise it rather than bumping it.
	//
	// The trap: naabu shares its entire transitive graph with nuclei --
	// projectdiscovery/utils, fastdialer, retryablehttp-go, mapcidr, gologger,
	// hmap, networkpolicy. Bumping nuclei moves all of those UNDER this pinned
	// naabu without changing this line, so "naabu is still v2.3.3" is NOT by
	// itself evidence that port scanning still works. The 2026-08 nuclei bump
	// (3.4.7 -> 3.8.0) moved projectdiscovery/utils v0.4.21 -> v0.10.1 exactly
	// this way.
	//
	// So: any change that touches nuclei or any projectdiscovery module must
	// re-run scripts/smoke/smoke.sh, whose section 2 is a real CONNECT scan
	// against local listeners. Compiling is not the bar; finding the open
	// ports and rejecting the closed control port is the bar.
...
require (
	...
	// HELD at v0.132.0 -- do not bump without also bumping nuclei.
	//
	// v0.144.0 changed Schema.ExclusiveMin/ExclusiveMax from bool to a pointer
	// type. nuclei's openapi input format does not compile against that, at
	// 3.4.7 or at 3.8.0 -- nuclei 3.8.0 still pins v0.132.0 itself. Bumping
	// this alone breaks the build with "non-boolean condition in if statement"
	// in nuclei/pkg/input/formats/openapi/examples.go.
	//
	// A scanner will flag this hold, including GHSA-r277-6w6q-xmqw (CRITICAL).
	// That is expected and was adjudicated on 2026-08-31: every kin-openapi
	// advisory is in openapi3filter, the server-side request-validation
	// package (ValidationHandler.Load fail-open auth bypass,
	// NoopAuthenticationFunc, nil-pointer panics in ConvertErrors).
	// networkscan never imports kin-openapi and never runs a validating
	// server; nuclei uses the openapi3 spec parser only. govulncheck does not
	// report kin-openapi as reachable. Forcing the bump breaks the build for
	// zero reachable benefit.
	//
	// Revisit when nuclei moves its own pin past v0.144.0.
	// github.com/oasdiff/yaml and yaml3 are held at their matching
	// pseudo-versions for the same reason -- newer ones drop
	// yaml.UnmarshalWithOrigin, which kin-openapi v0.132.0 needs.
