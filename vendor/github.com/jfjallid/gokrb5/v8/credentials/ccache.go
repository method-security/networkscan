...
func (c *CCache) SetKDCTimeOffset(seconds, microseconds uint32) {
	w := bytes.NewBuffer([]byte{})
	endian := c.getEndian()
	binary.Write(w, *endian, seconds)
	binary.Write(w, *endian, microseconds)
	c.Header.length = 12
	c.Header.fields = []headerField{headerField{tag: 1, length: 8, value: w.Bytes()}}
}
