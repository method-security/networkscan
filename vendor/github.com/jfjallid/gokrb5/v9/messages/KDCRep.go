...
func (k *ASRep) Verify(cfg *config.Config, aliases *config.RealmAliases, creds *credentials.Credentials, asReq ASReq) (bool, error) {
	...
	// The KDC may canonicalize the realm (e.g. return the DNS-style form when a
	// NetBIOS short name was requested). Compare alias-aware so a KDC-asserted
	// equivalence is accepted while genuinely unrelated realms are still rejected.
	if !config.RealmsEquivalent(aliases, k.CRealm, asReq.ReqBody.Realm) {
	...
	if !config.RealmsEquivalent(aliases, k.DecryptedEncPart.SRealm, asReq.ReqBody.Realm) {
...
func (k *TGSRep) Verify(cfg *config.Config, aliases *config.RealmAliases, tgsReq TGSReq) (bool, error) {
	if !k.CName.Equal(tgsReq.ReqBody.CName) {
		if tgsReq.ReqBody.CName.NameString != nil {
	...
	// Alias-aware comparison: the KDC may return the realm in a canonical form
	// differing from the requested one (e.g. a NetBIOS short name was used). A
	// genuine cross-realm referral is handled by the caller, which inspects the
	// service name separately.
	if !config.RealmsEquivalent(aliases, k.Ticket.Realm, tgsReq.ReqBody.Realm) {
	...
	//	}
	//}
	if !config.RealmsEquivalent(aliases, k.DecryptedEncPart.SRealm, tgsReq.ReqBody.Realm) {
