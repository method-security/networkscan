...
import (
	...
	"github.com/jfjallid/gokrb5/v9/pkinit"
...
func (cl *Client) ASExchangeExt(realm string, ASReq messages.ASReq, referral int, verify bool) (messages.ASRep, error) {
	...
	// RFC 6806 name canonicalization: the KDC may return the client realm in a
	// different (canonical, DNS-style) form than the one requested, most
	// commonly when the request used a NetBIOS short name. Treat the KDC's form
	// as authoritative and record the equivalence so the verification below and
	// every later realm lookup accept either form.
	if cl.aliases != nil && !config.EqualRealm(ASRep.CRealm, ASReq.ReqBody.Realm) {
		cl.AddRealmAlias(ASReq.ReqBody.Realm, ASRep.CRealm)
		log.Debugf("registered realm alias from AS-REP: %q -> %q\n", ASReq.ReqBody.Realm, ASRep.CRealm)
	}
	...
	if verify {
		if ok, err := ASRep.Verify(cl.Config, cl.RealmAliases(), cl.Credentials, ASReq); !ok {
...
func (cl *Client) processASRepPKINIT(asRep *messages.ASRep, asReq messages.ASReq) error {
	...
	// Accept (and learn) a KDC-canonicalized client realm, as in the non-PKINIT path.
	if cl.aliases != nil && !config.EqualRealm(asRep.CRealm, asReq.ReqBody.Realm) {
		cl.AddRealmAlias(asReq.ReqBody.Realm, asRep.CRealm)
		log.Debugf("registered realm alias from AS-REP (PKINIT): %q -> %q\n", asReq.ReqBody.Realm, asRep.CRealm)
	}
	if !config.RealmsEquivalent(cl.RealmAliases(), asRep.CRealm, asReq.ReqBody.Realm) {
