...
// If the kvno is zero then the latest matching kvno is returned; the selected kvno is returned alongside the key.
//
// If no entry matches the requested kvno exactly, the lookup is retried ignoring
// the kvno. AD stamps the AS_REP enc-part with the account's current key version,
// which a hand-built keytab often does not carry (e.g. one made with
// "ktutil addent" using the wrong -k value). MIT and Heimdal select the
// decryption key by principal/etype and tolerate a kvno mismatch here, so we do
// the same rather than failing the bind over a stale kvno label.
func (kt *Keytab) GetEncryptionKey(princName types.PrincipalName, realm string, kvno int, etype int32) (types.EncryptionKey, int, error) {
	key, kv := kt.findKey(princName, realm, kvno, etype)
	if len(key.KeyValue) < 1 && kvno != 0 {
		key, kv = kt.findKey(princName, realm, 0, etype)
	}
...
// findKey returns the newest key (by timestamp) matching the principal, realm and
// etype. A kvno of zero matches any key version; otherwise the kvno must match exactly.
func (kt *Keytab) findKey(princName types.PrincipalName, realm string, kvno int, etype int32) (types.EncryptionKey, int) {
...
// Principal returns the principal name and realm of the first entry in the keytab.
// Intended for deriving a login identity when a keytab holds the keys for a single
// principal. Returns an error if the keytab has no entries.
func (kt *Keytab) Principal() (types.PrincipalName, string, error) {
	if len(kt.Entries) == 0 {
		return types.PrincipalName{}, "", fmt.Errorf("keytab contains no entries")
	}
	e := kt.Entries[0]
	return types.PrincipalName{
		NameType:   e.Principal.NameType,
		NameString: e.Principal.Components,
	}, e.Principal.Realm, nil
}
