		...
		for _, ehlo := range []bool{false, true} {
			for _, short := range []string{"2", "22"} {
				t.Run(fmt.Sprintf("tls=%v/ehlo=%v/%s", secure, ehlo, short), func(t *testing.T) {
					...
					var result *discover.ServiceDetails
					if secure {
						result, _ = (&TLSPlugin{}).Run(client, time.Second, helpers.Endpoint{})
					} else {
						result, _ = (&SMTPPlugin{}).Run(client, time.Second, helpers.Endpoint{})
					}
					if result != nil {
