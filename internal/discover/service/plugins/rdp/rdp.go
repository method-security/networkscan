...
func parseRDPAuth(response []byte) (*ServiceRDP, bool, error) {
	info := ServiceRDP{}
...
func (p *RDPPlugin) Run(conn net.Conn, timeout time.Duration, target helpers.Endpoint) (*discover.ServiceDetails, error) {
...
func (p *TLSPlugin) Run(conn net.Conn, timeout time.Duration, target helpers.Endpoint) (*discover.ServiceDetails, error) {
	...
	if check && err != nil {
		...
		return helpers.MetadataResult(target, *info, true, "", common.TransportTypeTcp), nil
	}
	return nil, err
}
...
func (p *RDPPlugin) Type() common.TransportType {
...
var defaultRDPPluginPorts = helpers.Ports((&RDPPlugin{}).PortPriority)

func (p *RDPPlugin) DefaultPorts() []int { return defaultRDPPluginPorts }
func (p *RDPPlugin) Detect(ctx context.Context, ip net.IP, port int, host string, timeout int) (*discover.ServiceDetails, error) {
...
func (p *TLSPlugin) Detect(ctx context.Context, ip net.IP, port int, host string, timeout int) (*discover.ServiceDetails, error) {
	ctx, cancel := helpers.Context(ctx, timeout)
	defer cancel()
	conn, target, err := helpers.ConnectService(ctx, ip, port, host, timeout, common.TransportTypeTcp)
	if err != nil {
		return nil, err
	}
	defer func() { _ = conn.Close() }()
	...
	secure, err := helpers.UpgradeTLS(ctx, conn, host)
	if err != nil {
		return nil, err
	}
	conn = secure
