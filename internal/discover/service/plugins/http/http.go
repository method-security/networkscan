...
func (p *HTTPPlugin) Run(conn net.Conn, timeout time.Duration, target helpers.Endpoint) (*discover.ServiceDetails, error) {
	...
	return helpers.MetadataResult(target, payload, false, resp.Header.Get("Server"), common.TransportTypeTcp), nil
}
...
func (p *HTTPSPlugin) Run(
	...
	return helpers.MetadataResult(target, payload, true, resp.Header.Get("Server"), common.TransportTypeTcp), nil
}
func (p *HTTPPlugin) Type() common.TransportType {
	return common.TransportTypeTcp
}
func (p *HTTPSPlugin) Type() common.TransportType {
...
var defaultHTTPPluginPorts = helpers.Ports((&HTTPPlugin{}).PortPriority)

func (p *HTTPPlugin) DefaultPorts() []int { return defaultHTTPPluginPorts }
func (p *HTTPPlugin) Detect(ctx context.Context, ip net.IP, port int, host string, timeout int) (*discover.ServiceDetails, error) {
...
var defaultHTTPSPluginPorts = helpers.Ports((&HTTPSPlugin{}).PortPriority)

func (p *HTTPSPlugin) DefaultPorts() []int { return defaultHTTPSPluginPorts }
func (p *HTTPSPlugin) Detect(ctx context.Context, ip net.IP, port int, host string, timeout int) (*discover.ServiceDetails, error) {
	ctx, cancel := helpers.Context(ctx, timeout)
	defer cancel()
	conn, target, err := helpers.ConnectService(ctx, ip, port, host, timeout, p.Type())
	if err != nil {
