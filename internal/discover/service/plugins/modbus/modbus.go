...
*/
func (p *MODBUSPlugin) Run(conn net.Conn, timeout time.Duration, target helpers.Endpoint) (*discover.ServiceDetails, error) {
	...
	if bytes.Equal(response[:2], transactionID) {
		...
		if len(response) >= 10 && response[ModbusHeaderLength] == ModbusDiscreteInputCode {
			if response[ModbusHeaderLength+1] == 1 && (response[ModbusHeaderLength+2]>>1) == 0x00 {
				return helpers.MetadataResult(target, ServiceModbus{}, false, "", common.TransportTypeTcp), nil
			}
...
func (p *MODBUSPlugin) Type() common.TransportType {
...
var defaultMODBUSPluginPorts = helpers.Ports((&MODBUSPlugin{}).PortPriority)

func (p *MODBUSPlugin) DefaultPorts() []int { return defaultMODBUSPluginPorts }
func (p *MODBUSPlugin) Detect(ctx context.Context, ip net.IP, port int, host string, timeout int) (*discover.ServiceDetails, error) {
