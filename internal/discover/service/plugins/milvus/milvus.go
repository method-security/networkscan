...
//   - error: Error details if detection failed
func tryGetVersionViaHTTPREST(conn net.Conn, target helpers.Endpoint, timeout time.Duration) (string, bool, error) {
...
//   - error: Error details if detection failed
func DetectMilvus(conn net.Conn, timeout time.Duration, target helpers.Endpoint) (milvusMetadata, bool, error) {
...
func (p *MilvusPlugin) Run(conn net.Conn, timeout time.Duration, target helpers.Endpoint) (*discover.ServiceDetails, error) {
	metadata, detected, err := DetectMilvus(conn, timeout, target)
	if !detected {
		return nil, err
	}

	payload := ServiceMilvus{}

	cpe := buildMilvusCPE(metadata.Version)
	payload.CPEs = []string{cpe}

	return helpers.MetadataResult(target, payload, false, metadata.Version, common.TransportTypeTcp), nil
}
...
func (p *MilvusPlugin) Type() common.TransportType {
...
var defaultMilvusPluginPorts = helpers.Ports((&MilvusPlugin{}).PortPriority)

func (p *MilvusPlugin) DefaultPorts() []int { return defaultMilvusPluginPorts }
func (p *MilvusPlugin) Detect(ctx context.Context, ip net.IP, port int, host string, timeout int) (*discover.ServiceDetails, error) {
