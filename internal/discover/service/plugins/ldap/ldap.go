...
func (p *LDAPPlugin) Run(conn net.Conn, timeout time.Duration, target helpers.Endpoint) (*discover.ServiceDetails, error) {
	...
	if isLDAP {
		return helpers.MetadataResult(target, ServiceLDAP{}, false, "", common.TransportTypeTcp), nil
	}
	return nil, nil
}
...
func (p *LDAPPlugin) Type() common.TransportType {
...
func (p *TLSPlugin) Run(conn net.Conn, timeout time.Duration, target helpers.Endpoint) (*discover.ServiceDetails, error) {
	...
	if isLDAPS {
		return helpers.MetadataResult(target, ServiceLDAPS{}, true, "", common.TransportTypeTcp), nil
	}
	return nil, nil
}
...
var defaultLDAPPluginPorts = helpers.Ports((&LDAPPlugin{}).PortPriority)

func (p *LDAPPlugin) DefaultPorts() []int { return defaultLDAPPluginPorts }
func (p *LDAPPlugin) Detect(ctx context.Context, ip net.IP, port int, host string, timeout int) (*discover.ServiceDetails, error) {
