...
func (p *POSTGRESPlugin) Run(conn net.Conn, timeout time.Duration, target helpers.Endpoint) (*discover.ServiceDetails, error) {
	startupPacket := []byte{
...
func (p *POSTGRESPlugin) Type() common.TransportType {
...
var defaultPOSTGRESPluginPorts = helpers.Ports((&POSTGRESPlugin{}).PortPriority)

func (p *POSTGRESPlugin) DefaultPorts() []int { return defaultPOSTGRESPluginPorts }
func (p *POSTGRESPlugin) Detect(ctx context.Context, ip net.IP, port int, host string, timeout int) (*discover.ServiceDetails, error) {
