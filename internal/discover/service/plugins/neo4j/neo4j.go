...
func (p *NEO4JPlugin) Run(conn net.Conn, timeout time.Duration, target helpers.Endpoint) (*discover.ServiceDetails, error) {
	version, detected, err := DetectNeo4j(conn, timeout)
	if !detected {
		return nil, err
	}

	payload := ServiceNeo4j{}
	if version != "" {
...
func (p *NEO4JPlugin) Type() common.TransportType {
	return common.TransportTypeTcp
}
func (p *NEO4JPlugin) Priority() int {
	return -1
}
func (p *NEO4JTLSPlugin) Run(conn net.Conn, timeout time.Duration, target helpers.Endpoint) (*discover.ServiceDetails, error) {
	version, detected, err := DetectNeo4j(conn, timeout)
	if !detected {
		return nil, err
	}
...
func (p *NEO4JTLSPlugin) Type() common.TransportType {
...
var defaultNEO4JPluginPorts = helpers.Ports((&NEO4JPlugin{}).PortPriority)

func (p *NEO4JPlugin) DefaultPorts() []int { return defaultNEO4JPluginPorts }
func (p *NEO4JPlugin) Detect(ctx context.Context, ip net.IP, port int, host string, timeout int) (*discover.ServiceDetails, error) {
...
var defaultNEO4JTLSPluginPorts = helpers.Ports((&NEO4JTLSPlugin{}).PortPriority)

func (p *NEO4JTLSPlugin) DefaultPorts() []int { return defaultNEO4JTLSPluginPorts }
func (p *NEO4JTLSPlugin) Detect(ctx context.Context, ip net.IP, port int, host string, timeout int) (*discover.ServiceDetails, error) {
	ctx, cancel := helpers.Context(ctx, timeout)
	defer cancel()
	conn, target, err := helpers.ConnectService(ctx, ip, port, host, timeout, p.Type())
	if err != nil {
