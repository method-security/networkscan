...
func (p *MSSQLPlugin) Run(conn net.Conn, timeout time.Duration, target helpers.Endpoint) (*discover.ServiceDetails, error) {
	...
	payload := ServiceMSSQL{
		CPEs: []string{cpe},
	}

	return helpers.MetadataResult(target, payload, false, data.Version, common.TransportTypeTcp), nil
}
func (p *MSSQLPlugin) Name() string {
	return MSSQL
}
func (p *MSSQLPlugin) Type() common.TransportType {
...
var defaultMSSQLPluginPorts = helpers.Ports((&MSSQLPlugin{}).PortPriority)

func (p *MSSQLPlugin) DefaultPorts() []int { return defaultMSSQLPluginPorts }
func (p *MSSQLPlugin) Detect(ctx context.Context, ip net.IP, port int, host string, timeout int) (*discover.ServiceDetails, error) {
