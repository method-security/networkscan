...
func DetectMilvusMetrics(conn net.Conn, target probe.Target, timeout time.Duration) (string, bool, error) {
...
func (p *MilvusMetricsPlugin) Run(conn net.Conn, timeout time.Duration, target probe.Target) (*probe.Service, error) {
	version, detected, err := DetectMilvusMetrics(conn, target, timeout)
	if !detected {
		return nil, err
	}

	payload := probe.ServiceMilvusMetrics{}
...
func (p *MilvusMetricsPlugin) Type() probe.Protocol {
	return probe.TCP
}
...
var defaultMilvusMetricsPluginPorts = probe.Ports((&MilvusMetricsPlugin{}).PortPriority)

func (p *MilvusMetricsPlugin) DefaultPorts() []int { return defaultMilvusMetricsPluginPorts }
func (p *MilvusMetricsPlugin) Detect(ctx context.Context, ip net.IP, port int, host string, timeout int) (*probe.Service, error) {
