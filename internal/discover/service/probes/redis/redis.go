...
func (p *REDISTLSPlugin) Run(conn net.Conn, timeout time.Duration, target probe.Target) (*probe.Service, error) {
	return DetectRedis(conn, target, timeout, true)
}
func DetectRedis(conn net.Conn, target probe.Target, timeout time.Duration, tls bool) (*probe.Service, error) {
...
func (p *REDISPlugin) Run(conn net.Conn, timeout time.Duration, target probe.Target) (*probe.Service, error) {
...
func (p *REDISPlugin) Type() probe.Protocol {
	return probe.TCP
}
func (p *REDISTLSPlugin) Type() probe.Protocol {
	return probe.TCPTLS
}
...
var defaultREDISTLSPluginPorts = probe.Ports((&REDISTLSPlugin{}).PortPriority)

func (p *REDISTLSPlugin) DefaultPorts() []int { return defaultREDISTLSPluginPorts }
func (p *REDISTLSPlugin) Detect(ctx context.Context, ip net.IP, port int, host string, timeout int) (*probe.Service, error) {
	return probe.Detect(ctx, ip, port, host, timeout, p.Name(), p.Type(), p.Run)
}

var defaultREDISPluginPorts = probe.Ports((&REDISPlugin{}).PortPriority)

func (p *REDISPlugin) DefaultPorts() []int { return defaultREDISPluginPorts }
func (p *REDISPlugin) Detect(ctx context.Context, ip net.IP, port int, host string, timeout int) (*probe.Service, error) {
	return probe.Detect(ctx, ip, port, host, timeout, p.Name(), p.Type(), p.Run)
}
