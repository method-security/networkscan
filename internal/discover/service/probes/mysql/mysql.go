...
func (p *MYSQLPlugin) Run(conn net.Conn, timeout time.Duration, target probe.Target) (*probe.Service, error) {
	...
	if err == nil {
		...
		payload := probe.ServiceMySQL{
			PacketType:   "handshake",
			ErrorMessage: "",
			ErrorCode:    0,
			CPEs:         []string{cpe},
		}
		return probe.Result(target, payload, false, mysqlVersionStr, probe.TCP), nil
	}
...
func (p *MYSQLPlugin) Type() probe.Protocol {
	return probe.TCP
}
...
var defaultMYSQLPluginPorts = probe.Ports((&MYSQLPlugin{}).PortPriority)

func (p *MYSQLPlugin) DefaultPorts() []int { return defaultMYSQLPluginPorts }
func (p *MYSQLPlugin) Detect(ctx context.Context, ip net.IP, port int, host string, timeout int) (*probe.Service, error) {
