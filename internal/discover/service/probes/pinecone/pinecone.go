...
//
// Returns:
//   - *probe.Service with Pinecone detection if headers present
//   - nil if not detected
//   - error on request failures (connection issues, timeouts)
func (p *PINECONEPlugin) Run(conn net.Conn, timeout time.Duration, target probe.Target) (*probe.Service, error) {
...
func (p *PINECONEPlugin) buildDetectionResult(
	...
	payload := probe.ServicePinecone{
		CPEs: []string{cpe},

		APIVersion: apiVersion,
	}

	return probe.Result(target, payload, true, "", probe.TCPTLS), nil
}
...
func (p *PINECONEPlugin) Type() probe.Protocol {
	return probe.TCPTLS
}
...
var defaultPINECONEPluginPorts = probe.Ports((&PINECONEPlugin{}).PortPriority)

func (p *PINECONEPlugin) DefaultPorts() []int { return defaultPINECONEPluginPorts }
func (p *PINECONEPlugin) Detect(ctx context.Context, ip net.IP, port int, host string, timeout int) (*probe.Service, error) {
