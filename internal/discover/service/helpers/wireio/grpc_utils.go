...
func GRPCDialWithTimeout(ctx context.Context, target string, timeout time.Duration) (*grpc.ClientConn, error) {
	...
	conn, err := grpc.DialContext(ctx, target,
		grpc.WithContextDialer(func(ctx context.Context, address string) (net.Conn, error) {
			return netdial.DialContext(ctx, "tcp", address, netdial.WithTimeout(timeout))
		}),
...
func GRPCInvokeUnary(ctx context.Context, conn *grpc.ClientConn, method string, request []byte, timeout time.Duration) ([]byte, error) {
	ctx, cancel := helpers.ContextDuration(ctx, timeout)
	defer cancel()

	var response []byte
	err := conn.Invoke(ctx, method, request, &response, grpc.ForceCodec(rawCodec{}))
	if err != nil {
...
func (rawCodec) Marshal(v interface{}) ([]byte, error) {
	b, ok := v.([]byte)
	if !ok {
		return nil, fmt.Errorf("expected protobuf bytes")
	}
	return b, nil
}
func (rawCodec) Unmarshal(b []byte, v interface{}) error {
	p, ok := v.(*[]byte)
	if !ok {
		return fmt.Errorf("expected protobuf destination")
	}
	*p = append((*p)[:0], b...)
	return nil
}
