...
// Mirrors internal/enumerate/smtp/helpers.go::tryTLSConnection while also
// propagating the caller's context deadline so the dial can be bounded by the
// engine's per-target timeout.
